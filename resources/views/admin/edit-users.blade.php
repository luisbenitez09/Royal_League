...
<body class="bg-dash bg-cover bg-center">
    <div class="min-h-screen">
        ...
        <div class="mx-8 lg:mx-20 flex flex-row justify-between my-8">
            <h1 class="text-white font-bold text-3xl">Usuario</h1>
        ...
        <div class="w-full">
            <div class="max-w-6xl xl:mx-auto lg:mx-20 bg-gray-500 bg-opacity-25 rounded-3xl mb-20 mx-8 p-10">
                <form method="POST" action="{{ route('changeStatus') }}"><!--cambiar ruta-->
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-2">
                        <h1 class="text-center md:text-left mb-10 text-xl md:text-2xl text-white font-light mr-5">Nombre: <span class="font-bold">{{ $user->name }}</span></h1>
                        <div class="flex flex-row lg:flex-row-reverse mb-4 md:mb-0">
                            <h1 class="text-center text-xl md:text-2xl text-white font-light mr-4 lg:mr-0">Puntos: <span class="font-bold">{{ $userPoints }}</span></h1>
                            ...
                            @php
                                if ($user->status === 0) {
                                    echo '<p class="text-red-600 text-center text-lg md:text-xl mt-1 font-light mx-2">Usuario suspendido</p>';
                                } else if ($user->status === 1) {
                                    echo '<p class="text-green-400 text-center text-lg md:text-xl mt-1 font-light mx-2">Usuario activo</p>';
                                }
                            @endphp
                        </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between">
                        <h1 class="mb-4 md:mb-0 text-xl md:text-2xl text-white font-light mr-5">Podios: <span class="font-bold">{{ $user->podiums }}</span></h1>
        ...
        <!-- Titulo -->
        <div class="mx-8 lg:mx-20 flex flex-row justify-between my-8">
            <h1 class="text-white font-bold text-3xl">Perfiles</h1>
        </div>
                ...
                <div class="max-w-6xl xl:mx-auto lg:mx-20 bg-gray-500 bg-opacity-25 rounded-3xl my-20 mx-8 p-10">
                    <table class="w-full text-white">
                        <thead class="text-xl border-b-2">
                            <th class="w-1/4 pb-4">Gamertag</th>
                        ...
                        <tbody>
                            @foreach ($profiles as $profile)
                                <tr class="">
                                    <td class="py-4 text-center">{{ $profile->gamertag }}</td>
                                    ...
                                    <td class="py-4 text-center">{{ $profile->points }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <!-- Titulo -->
        <div class="mx-8 lg:mx-20 flex flex-row justify-between my-8">
            <h1 class="text-white font-bold text-3xl">Equipos</h1>
        </div>
        ...
        <div class="w-full">
                <div class="max-w-6xl xl:mx-auto lg:mx-20 bg-gray-500 bg-opacity-25 rounded-3xl my-20 mx-8 p-10">
                    <table class="w-full text-white">
                        <thead class="text-xl border-b-2">
                            <th class="w-1/4 pb-4">Nombre</th>
                            <th class="w-1/4 pb-4">Puntos</th>
                        </thead>
                        <tbody>
                            @foreach ($teams as $team)
                                <tr class="">
                                    <td class="py-4 text-center">{{ $team->name }}</td>
                                    <td class="py-4 text-center">{{ $team->points }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
